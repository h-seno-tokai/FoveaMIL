...
from foveamil.models.heads import build_classifier_head
...
class FoveaMIL(nn.Module):
    ...
    Args:
        ...
        head_type: 識別器ヘッド名（``build_classifier_head`` のレジストリ）
        head_kwargs: 識別器ヘッドへ渡す追加引数
    ...
    def __init__(
        ...
        self.head = build_classifier_head(
            head_type, self.fusion.out_dim, n_cls, **(head_kwargs or {})
        )
