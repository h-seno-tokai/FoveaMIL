...
| `losses.py` | クラス不均衡に対応する分類損失（`build_loss`）． |
| `minority.py` | 少数クラスの学習信号を強化する機構（bag 表現 mixup・サンプラ温度・ordinal 補助損失，いずれも既定 off）． |
...
インスタンス補助損失（`instance_loss` `bag_weight` `inst_k` `inst_subtyping`）は，
...
不均衡対応の分類損失（`loss_type` `loss_tau` `loss_cb_beta` `loss_ldam_max_margin`）は
`losses.py` から構築する．既定 `loss_type="plain"` は素 cross-entropy で従来挙動と一致する．
`"logit_adjusted"`（事前確率 `π_y` の対数を `τ·log π_y` で logit に加算）・`"ldam"`（頻度
`n_y` 依存マージン）・`"class_balanced"`（有効標本数 `(1-β^n)/(1-β)` の逆数でクラス重み）を
選べる．クラス頻度は train split から自動算出する．`loss_tau` / `loss_cb_beta` /
`loss_ldam_max_margin` はそれぞれ `logit_adjusted` / `class_balanced` / `ldam` のときのみ
意味を持ち，他の `loss_type` では sweep が既定値へ畳んで記録しない．

少数クラスの学習信号を強化する 3 機構（`mixup_alpha` `sampler_temp` `ordinal_aux_weight`）は
`minority.py` から構築する．いずれも **既定値で従来挙動と数値一致**する（`mixup_alpha=0` /
`sampler_temp=1.0` / `ordinal_aux_weight=0`）．`mixup_alpha>0` は融合後 bag 表現
（`[B, dim]`）レベルの manifold-mixup で，バッチサイズ 1 のため直前サンプルの表現/ラベルと
線形補間し，ラベルも同じ比で補間する（補間係数 λ は seed から決定的に引く）．`sampler_temp`
は `is_weighted_sampler` 時のサンプラ重みを `weight^temp` に温める（`<1` で一様寄りに緩和，
`>1` で少数クラスを強調）．`ordinal_aux_weight>0` はクラス index の並びを順序とみなす ordinal
補助損失（softmax 確率の期待ランクと正解ランクの正規化二乗距離）を分類損失へ加える．

探索ベースのズーム（`zoom_driver` `mcts_planner` `mcts_simulations` `mcts_max_considered`
`policy_loss_weight` `value_loss_weight` `policy_entropy_weight` `mcts_hidden_dim`
`mcts_value_target` `mcts_rollout_depth` `mcts_eval_stochastic`）は，倍率ごとの
ズーム先を一括 top-k ではなく学習方策・価値による探索で決める．既定 `zoom_driver="differentiable"`
は従来の微分可能 top-k 駆動を再現する．`mcts_rollout_depth=1`（既定）は選んだ親の子を 1 段だけ
次倍率へ射影して価値ネットで評価する（look-ahead）．`>1` は子を更に次倍率へ再帰展開し（各段で
プランナが 1 子を選ぶ）最深状態を葉評価にする＝rollout の深さで先読みを伸ばす．子ロードは木全体で
キャッシュを共有し重複 h5 読みを抑える．`mcts_eval_stochastic=True` は葉評価を MC dropout で確率化し
報酬 memoize を撤廃して simulation 間に分散を出す（既定 `False` は eval モード葉評価＋memoize で
simulation 間同値）．これらは **`zoom_driver="mcts"` の多倍率のみ意味を持ち**，
それ以外（単一倍率，または既定駆動）では sweep が既定値へ畳んで記録しない．

## `losses.py` — 不均衡対応の分類損失

`build_loss(loss_type, class_frequencies, tau=, beta=, ldam_max_margin=)` が損失種別名と
クラス頻度から `(logits[B,C], target[B])` を取る損失モジュールを構築する．`"plain"` は
`nn.CrossEntropyLoss`（素 CE），`"logit_adjusted"` は `LogitAdjustedLoss`，`"ldam"` は
`LDAMLoss`，`"class_balanced"` は `ClassBalancedLoss` を返す．いずれも頻度依存の補正を
登録バッファに持ち `nn.functional.cross_entropy` へ委ねる．`Trainer` は train split の
クラス件数を `_class_frequencies` で算出して criterion を構築し，train/val/test 共通に使う．
...
precision・recall / kappa（重みなし）/ per-class f1・precision・recall を返す．
...
`ReduceLROnPlateau(mode='min')` を用いる．分類損失は `config.loss_type` から
`build_loss`（`losses.py`）で構築し（train split の頻度で初期化），train/val/test 共通に使う．
既定 `loss_type="plain"` では素 cross-entropy と一致する．
...
展開後，**構成に無関係な条件付きパラメータを既定値へ畳んで重複 combo を統合する**（直積の無駄を断つ）:
単一倍率では `k_sample` / `k_sigma` / `topk_method`（ズーム系）が無関係なので畳んで記録しない，
`instance_loss=True` は単一倍率のみ有効なので多倍率の combo では無効化する，`instance_loss=False`
では `bag_weight` / `inst_k` / `inst_subtyping` が無関係なので畳む．統合・除外が起きると警告を出す
（畳んだキーは結果表・集計に載せない）．多倍率ではズーム系をそのまま直積展開する．
